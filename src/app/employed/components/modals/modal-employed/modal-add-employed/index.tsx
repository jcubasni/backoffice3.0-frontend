...
import { CreateEmployedSchema, createEmployedSchema } from "@/app/employed/schemas/create-employed.schema"
import { employedTabs } from "@/app/employed/lib/employed-tabs"

import { SidebarEmployed } from "./sidebar-employed"

import { Modals } from "@/app/employed/types/modals-name"
import { useAddEmployed } from "@/app/employed/hooks/useEmployedsService"
...
export default function ModalAddEmployed() {
  const [activeTab, setActiveTab] = useState("misDatos")

  const addEmployed = useAddEmployed()

  const form = useForm<CreateEmployedSchema>({
    resolver: zodResolver(createEmployedSchema),
    mode: "onChange",
  })

  const handleSubmit = (data: CreateEmployedSchema) => {
    console.log("Empleado Registrado:", data)
    addEmployed.mutate(data)
  }
